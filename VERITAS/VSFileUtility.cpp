...
void VSFileUtility::expandFilename(std::string& filename, bool expand_env)
{ 
  ...
  if((!filename.empty())&&(expand_env))
    {
      unsigned lpos = 0;
      while(lpos<filename.length())
	{
	  if(filename[lpos]=='$')
	    {
	      std::string val;
	      ...
	      if(rpos==filename.length())
		{
		  filename.replace(lpos,rpos-lpos,val);
		  continue;
		}
	      ...
	      else if(filename[rpos]=='(')
		{
	      ...
	      std::string name;
	      while(rpos<filename.length())
		{		  
		  if(bracket && filename[rpos]==bracket)
		    {
		      bracket=0;
		      rpos++;
		      break;
		    }
		  else if(!isalnum(filename[rpos])&&(filename[rpos]!='_'))
		    {
		      break;
		    }
		  else
