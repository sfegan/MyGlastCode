...
namespace VERITAS
{
  ...
  public:
    ...
    class iterator: public std::iterator<std::random_access_iterator_tag, count_type, int, iterator*, count_type>
    {
    public:
      ...
      count_type operator*() const { return count(); }
    ...
    {
    public:
      count_type operator() (const T& bin_center, const count_type& bin_count)
      {
	return bin_count;
      }
    };
    ...
    public:
      count_type operator() (const T& bin_center, const count_type& bin_count)
      {
	return std::sqrt(bin_count);
      }
    };

    template<typename Op> inline void transform(Op op);
    template<typename Op> inline void injest(const VSSimpleHist& o, Op op);
    ...
    inline VSSimpleHist& operator+= (count_type x);
    inline VSSimpleHist& operator-= (count_type x);
    inline VSSimpleHist& operator*= (count_type x);
    inline VSSimpleHist& operator/= (count_type x);
    ...
    // ------------------------------------------------------------------------
    // Various more complex operations
    // ------------------------------------------------------------------------

    T mean() const;
    count_type sum() const;
    VSSimpleHist makeIntegralHist() const;
    VSSimpleHist<T,double,BIN> makeNormIntegralHist() const;
    T integralCountToVal(const count_type& y) const; // Must be well ordered!
  ...
  template<typename T, typename count_type, typename BIN> 
  template<typename Op> inline void VSSimpleHist<T,count_type,BIN>::
  transform(Op op)
  {
    for(iterator ibin = begin(); ibin!=end(); ibin++)
      setBin(ibin.bin(), op(ibin.center(), ibin.count()));
  }

  template<typename T, typename count_type, typename BIN> 
  template<typename Op> inline void VSSimpleHist<T,count_type,BIN>::
  injest(const VSSimpleHist& o, Op op)
  {    
    for(iterator ibin = o.begin(); ibin!=o.end(); ibin++)
      accumulate(ibin.center(), op(ibin.center(), ibin.count()));
  }
  ...
  template<typename T, typename count_type, typename BIN> 
  inline VSSimpleHist<T,count_type,BIN>& 
  VSSimpleHist<T,count_type,BIN>::operator+= (count_type x)
  {
    for(iterator ibin = begin(); ibin!=end(); ibin++)
  ...
  template<typename T, typename count_type, typename BIN> 
  VSSimpleHist<T,count_type,BIN> VSSimpleHist<T,count_type,BIN>::
  makeIntegralHist() const
  {
    std::string the_name(name());
    if(!the_name.empty())the_name += std::string(" (integral)");
    VSSimpleHist ih(fBinCalc.binWidth(), fBinCalc.binStart(), the_name);
    ih.accumulate(val(firstBin()-1), 0);
    count_type csum = count_type();
    for(iterator ibin=begin(); ibin!=end();ibin++)
      {
	csum += ibin->count();
	ih.accumulate(ibin->center(), csum);
      }
    return ih;
  }

  template<typename T, typename count_type, typename BIN> 
  VSSimpleHist<T,double,BIN> VSSimpleHist<T,count_type,BIN>::
  makeNormIntegralHist() const
  {
    std::string the_name(name());
    if(!the_name.empty())the_name += std::string(" (norm integral)");
    VSSimpleHist<T,double,BIN> ih(fBinCalc.binWidth(), fBinCalc.binStart(), 
				  the_name);
    ih.accumulate(val(firstBin()-1), 0);
    double sum = 0.0;
    for(iterator ibin=begin(); ibin!=end();ibin++)sum += double(ibin->count());
    double csum = 0.0;
    for(iterator ibin=begin(); ibin!=end();ibin++)
      {
	csum += double(ibin->count());
	ih.accumulate(ibin->val(), csum/sum);
      }
    return ih;
  }

  template<typename T, typename count_type, typename BIN> 
  T VSSimpleHist<T,count_type,BIN>::
  integralCountToVal(const count_type& y) const
  {
    iterator ifind = std::lower_bound(begin(), end(), y);
    if(ifind==end())
      throw std::out_of_range("integralCountToVal: value past end");
    if(ifind==begin())
      throw std::out_of_range("integralCountToVal: value past beginning");
    T x0 = ifind->val();
    count_type y1 = *(ifind--);
    count_type y0 = *ifind;  // This succeeds even if ifind == begin();
    return x0 + (y-y0)/(y1-y0)*fBinCalc.binWidth();
  }
  ...
  public:
    ...
    VSLimitedHist makeIntegralHist() const;
    VSLimitedHist<T,double,BIN> makeNormIntegralHist() const;
    T integralCountToVal(const count_type& y) const; // Must be well ordered!
  ...
  template<typename T, typename count_type, typename BIN> 
  VSLimitedHist<T,count_type,BIN> VSLimitedHist<T,count_type,BIN>::
  makeIntegralHist() const
  {
    std::string the_name(name());
    if(!the_name.empty())the_name += std::string(" (integral)");
    VSLimitedHist ih(binSize(), fLoLimit, fHiLimit, false, the_name);
    if(fUnderflow != count_type())
      for(unsigned ibin=bin(fLoLimit)-1;ibin<firstBin();ibin++)
	ih.accumulate(val(ibin), fUnderflow);
    else
      ih.accumulate(val(firstBin()-1), count_type());
    count_type csum = fUnderflow;
    for(iterator ibin=begin(); ibin!=end();ibin++)
      {
	csum += ibin->count();
	ih.accumulate(ibin->center(), csum);
  ...
  template<typename T, typename count_type, typename BIN> 
  VSLimitedHist<T,double,BIN> VSLimitedHist<T,count_type,BIN>::
  makeNormIntegralHist() const
  {
    std::string the_name(name());
    if(!the_name.empty())the_name += std::string(" (norm integral)");
    VSLimitedHist<T,double,BIN> ih(binSize(), fLoLimit, fHiLimit, 
				    false, the_name);
    double sum = double(fUnderflow) + double(fOverflow);
    for(iterator ibin=begin(); ibin!=end();ibin++)sum += double(ibin->count());
    if(double(fUnderflow) != 0.0)
      for(unsigned ibin=bin(fLoLimit)-1;ibin<firstBin();ibin++)
	ih.accumulate(val(ibin), double(fUnderflow)/sum);
    else
      ih.accumulate(val(firstBin()-1), 0.0);
    double csum = double(fUnderflow);
    for(iterator ibin=begin(); ibin!=end();ibin++)
      {
	csum += double(ibin->count());
	ih.accumulate(ibin->val(), csum/sum);
  ...
  template<typename T, typename count_type, typename BIN> 
  T VSLimitedHist<T,count_type,BIN>::
  integralCountToVal(const count_type& y) const
  {
    iterator ifind = std::lower_bound(begin(), end(), y);
    if(ifind==end())
