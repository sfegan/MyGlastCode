...
RotationVec3D& RotationVec3D::operator &= (const RotationVec3D& r)
{
#define R1 (*this)
#define R2 r

  const double r1_theta = R1.theta();
  const double r2_theta = R2.theta();
  ...
  if(r1_theta == 0.0)
  ...
  else
    {
      ...
      const Vec3D r1_hat = R1.axis();
      const Vec3D r2_hat = R2.axis();
      ...
      else
	{
	  ...
	  *this = axis;
	  fSinTheta = 2.0*sinhalftheta*coshalftheta;
	  fCosTheta = coshalftheta*coshalftheta-sinhalftheta*sinhalftheta;
	}
    }
  return *this;
}

RotationVec3D RotationVec3D::makeRotation(const Vec3D& v1, const Vec3D& v2, 
					  Subspace2D subspace)
{
  Vec3D ea(v1);
  ea.normalize();
