...
namespace VERITAS
{
  ...
  template<typename T> class VSSimpleStat1
  {
  public:
    VSSimpleStat1(): fCount(0), fSum(0) { /* nothing to see here */ }

    void accumulate(const T& x) { fCount++; fSum+=x; }
    void accumulate(const T& x, unsigned n) { fCount+=n, fSum+=x*T(n); }
  ...
  template<typename T> VERITAS::VSSimpleStat1<T>& VSSimpleStat1<T>::
